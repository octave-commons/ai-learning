...
export class ModelRouter {
    ...
    routingHistory = [];
    defaultStrategy;
    constructor(performanceTracker, defaultStrategy = 'balanced') {
     ...
     */
    async selectBestModel(prompt, availableModels, strategy, context) {
    ...
    getRoutingStatistics() {
        ...
        return {
            ...
            averageConfidence: this.routingHistory.length > 0 ? totalConfidence / this.routingHistory.length : 0,
     ...
     */
    selectModelByStrategy(performances, strategy) {
        ...
        switch (strategy) {
            ...
            case 'fastest':
                return performances.sort((a, b) => a.capabilities.speed.localeCompare(b.capabilities.speed))[0];
            case 'cheapest':
                return performances.sort((a, b) => a.capabilities.costPerToken - b.capabilities.costPerToken)[0];
            case 'most-reliable':
                return performances.sort((a, b) => b.capabilities.reliability - a.capabilities.reliability)[0];
            case 'balanced':
     ...
     */
    calculateRoutingConfidence(selectedModel, allPerformances) {
     ...
     */
    generateReasoning(selectedModel, strategy, taskCategory) {
        const reasons = [];
        switch (strategy) {
            case 'best-performance':
                reasons.push(`Selected for highest performance score (${selectedModel.averageScore.toFixed(2)})`);
                break;
            case 'fastest':
                reasons.push(`Selected for fastest response speed (${selectedModel.capabilities.speed})`);
                break;
            case 'cheapest':
                reasons.push(`Selected for lowest cost per token ($${selectedModel.capabilities.costPerToken.toFixed(6)})`);
                break;
            case 'most-reliable':
                reasons.push(`Selected for highest reliability (${(selectedModel.capabilities.reliability * 100).toFixed(1)}%)`);
                break;
            case 'balanced':
     ...
     */
    assessRiskLevel(performance, confidence) {
